...
%% Construct the MDP
Transition=parse_transition_ltl_human();%Transition(from,to,act)=p(from,act,to)
M.state_no = Task.state_no*Fatigue.state_no;
M.S = states_human(); 
...
M.CONTROLSTRING{1} = 'a0h'; M.CONTROLSTRING{4} = 'a3h'; 
M.CONTROLSTRING{2} = 'a1h'; M.CONTROLSTRING{5} = 'reset'; 
M.CONTROLSTRING{3} = 'a2h';
...
P.Spi = [];
Cost=cost_assignment_human();%Cost(s,a)
